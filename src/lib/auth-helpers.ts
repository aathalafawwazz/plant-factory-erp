...
/**
 * The profile view used everywhere in role/status-gated code. This is a
 * thin projection of `profiles` — only the fields middleware and
 ...
 *
 * This is the single source of truth for server-side auth checks.
 * Components should use `<CurrentUserProvider>` + `useCurrentUser()`
 * on the client instead of calling this directly.
 */
...
export const ROUTE_ROLE_GATES: Array<{
  ...
  // Sales — admin + operator (viewer can read sales_orders via RLS but
  // we hide the whole section from the sidebar)
  { prefix: "/sales",        allowed: ["admin", "operator", "viewer"],    description: "Sales (viewer read-only)" },
  // Inventory — admin + operator + viewer (read)
